    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          role="combobox"
          aria-expanded={open}
          className="w-[200px] justify-between bg-white/5 border-white/10 text-white hover:bg-white/10 hover:text-white"
        >
          {value
              ...
              {options.map((option) => (
                ...
                >
                  <Check
                    className={cn(
                      "mr-2 h-4 w-4",
                      value === option.value ? "opacity-100 text-[#39ff14]" : "opacity-0"
                    )}
                  />
                  {option.label}
                </CommandItem>
